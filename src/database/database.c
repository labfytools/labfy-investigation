 ...
 */
#define DATABASE_SCHEMA_VERSION_CURRENT 20

/**
 * @brief Version actuelle sous forme textuelle pour metadata.
 */
#define DATABASE_SCHEMA_VERSION_CURRENT_TEXT "20"
...
rollback:
    if (transaction_started && !database_transaction_rollback(database))
        g_warning("Impossible d’annuler la migration SQLite V2 vers V3.");
    return false;
}
...
rollback:
    if (transaction_started && !database_transaction_rollback(database))
        g_warning("Impossible d’annuler la migration SQLite V3 vers V4.");
    return false;
}
...
rollback:
    if (transaction_started && !database_transaction_rollback(database))
        g_warning("Impossible d’annuler la migration SQLite V9 vers V10.");
    return false;
}
...
rollback:
    if (transaction_started && !database_transaction_rollback(database))
        g_warning("Impossible d’annuler la migration SQLite V19 vers V20.");
    return false;
}
